...
__global__ void DepthConvFused_2_kernel0(const float* Input, 
  ...
  // float *Input_shared = s;
  // float *intermediate = &s[STEP_READ_TILE_H * STEP_READ_TILE_W * IC_stride];
  // float *Conv2dFilter_1_shared = &s[STEP_READ_TILE_H * STEP_READ_TILE_W * IC_stride + OUTPUT_SIZE_HW * IC_stride];
  ...
  float *Conv2dFilter_1_shared = &s[OUTPUT_SIZE_HW * IC_stride];
	...
	do {
		...
		int global_h_array[(STEP_PER_ROUND_CTA + 1) / 2 * 2] = { -1 };
  		int global_w_array[(STEP_PER_ROUND_CTA + 1) / 2 * 2] = { -1 };
		...
		for (int _g_ic_step = 0; _g_ic_step < (IC / IC_stride); _g_ic_step++) {
			...
			for (int loop = 0; loop < STEP_PER_ROUND_CTA; loop++) {
				...
				loadFloat2(buffer, Conv2dFilter_1_shared, 0, _s_coord);
				__syncthreads();
			...
			// if (blx == 0 && thy == 2 && thx == 0 && _g_ic_step == 0) {
			// 	for (int i = 0; i < (STEP_PER_ROUND_CTA + 1) / 2 * 2; i++) {
			// 		printf("i: %d, global_h: %d, global_w: %d\n", i, global_h_array[i], global_w_array[i]);
			// 	}
			// }
			...
			for (int iter = 0; iter < OC / OC_stride; iter++) {
			  ...
			  for (int i = 0, 
			  			inter_offset = thy * 64 + (thx / OC_STRIDE_SPLIT) * IC_stride,
			  			filter_offset = (thx % OC_STRIDE_SPLIT);
			  		i < IC_stride; 
			  		i++,
			  			inter_offset++,
			  			filter_offset += OC_stride) {

				// 8 = BLOCK_DIM_Y * 32 / OC_STRIDE_SPLIT, which is basically fixed
				#pragma unroll
				for (int j = 0, a = iter * 2, b = inter_offset;
					j < (OUTPUT_SIZE_HW - 1) / 8 + 1; // 28 -> 4, 32 -> 4
