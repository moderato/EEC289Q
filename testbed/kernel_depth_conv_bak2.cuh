...
extern "C" __global__ void DepthConvFused_2_kernel0( float* __restrict__ Input,  float* __restrict__ DepthwiseFilter_1,  float* __restrict__ Conv2dFilter_1,  float* __restrict__ Conv2dOutput_0) {
  ...
  for (int rc_outer_v = 0; rc_outer_v < 4; ++rc_outer_v) {
    ...
    for(int iter = 0; iter < 32; ++iter) {
      float tmp = Conv2dFilter_1_shared[((int)threadIdx.x) * 32 + iter];
      red_buf = DepthwiseConv2dOutput_0_local[0] * tmp;
  ...
  Conv2dOutput_0[(((((((((int)blockIdx.x) / 112) * 14336) + ((((int)blockIdx.x) % 4) * 32)) + (((((int)blockIdx.x) / 4) % 28) * 256)) + ((((int)threadIdx.y) / 2) * 7168)) + ((((int)threadIdx.y) % 2) * 128)) + ((int)threadIdx.x))] = red_buf0[((int)threadIdx.y) * 32 + ((int)threadIdx.x)];
}
