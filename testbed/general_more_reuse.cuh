...
extern "C" __global__ void DepthConvFused_2_kernel0(const float* Input, 
  ...
  for (int rc_outer_v = 0; rc_outer_v < (int)(C / C_stride); ++rc_outer_v) {
    ...
    *********************/
    ...
    #pragma unroll
        for (int i = 0; i < 32; i++) {
          // (thx / 16) * C_stride causes weird slowdown!!
          Conv2dOutput_0_local[4 * j + iter] += intermediate[128 * j + (thy / 2) * 64 + (thx / 16) * 32 + i] * Conv2dFilter_1_shared[(thx % 16) + C_stride * i + (thy % 2) * 16];
        }
      }
